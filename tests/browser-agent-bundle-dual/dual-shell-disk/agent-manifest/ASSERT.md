...
- `Chrome-Ext-Browser-Agent` exists under ModuleRoot.
- `manifest.json` found (public/ or root or src/).
- Manifest references Browser Agent (name/description; case-insensitive OK).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !strings.Contains(low, "browser agent") && !strings.Contains(low, "browser-agent") {
		t.Fatalf("manifest name/description should reference Browser Agent; path=%s manifest=%s",
			resp.ManifestPath, truncate(resp.ManifestText, 400))
	}
	if !strings.Contains(resp.ManifestText, "43761") {
		t.Fatalf("manifest must mention host port 43761; path=%s manifest=%s",
			resp.ManifestPath, truncate(resp.ManifestText, 400))
	}
}
```
