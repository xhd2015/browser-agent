...
const (
	DefaultDevBackendPort = 18719
	FrontendDevPort       = 18720
)
...
func reclaimPort(port int) error {
	if checkPort(port) {
		fmt.Printf("Killing existing listener(s) on port %d...\n", port)
		if err := killPortListeners(port); err != nil {
			return err
		}
		for i := 0; i < 20; i++ {
...
func reclaimFrontendDevPort() error {
	return reclaimPort(FrontendDevPort)
}

func killPortListeners(port int) error {
	...
	for _, line := range strings.Split(pidStr, "\n") {
		...
		fmt.Printf("Killing process %d occupying port %d\n", pid, port)
...
func startDevFrontend(ctx context.Context, httpServer *http.Server) (chan struct{}, error) {
	if err := reclaimFrontendDevPort(); err != nil {
		return nil, err
	}
	...
	return EnsureFrontendDevServer(ctx)
}
...
func EnsureFrontendDevServer(ctx context.Context) (chan struct{}, error) {
	...
	cmd.Dir = "project-api-capture-react/"
	...
	fmt.Println()
	return nil, fmt.Errorf("frontend dev server failed to start on port %d within timeout (strictPort is enabled)", FrontendDevPort)
}

func Serve(port int, dev bool) error {
	if dev {
		if err := reclaimPort(port); err != nil {
			return err
		}
	}
	...
	if dev {
		...
		subProcessDone, err := startDevFrontend(ctx, server)
		if err != nil {
			return err
		}
...
func ProxyDev(mux *http.ServeMux) error {
	targetURL, err := url.Parse(fmt.Sprintf("http://localhost:%d", FrontendDevPort))
	if err != nil {
...
func Static(mux *http.ServeMux, opts StaticOptions) error {
	...
	reactFileSystem, err := fs.Sub(distFS, "project-api-capture-react/dist")
	if err != nil {
	...
	mux.Handle("/project-api-capture.svg", &mimeTypeHandler{http.FileServer(http.FS(reactFileSystem))})
