...
- Stdout contains status output with **not running** indication.
...
- Non-nil CLI error, missing stdout, or meta creation fails.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if strings.Contains(low, "running: true") || strings.Contains(low, "running:true") {
		t.Fatalf("stdout claims running=true without daemon; stdout=%q", truncate(resp.Stdout, 600))
	}
	if strings.Contains(low, "running: yes") {
		t.Fatalf("stdout claims running=yes without daemon; stdout=%q", truncate(resp.Stdout, 600))
	}
