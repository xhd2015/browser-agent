...
**Feature**: HandleCLI dispatch for full command surface without live control server

```
# Dispatch-only: no listen socket, no Chrome
Operator -> HandleCLI(args, empty env, stdout, stderr)
  --help / -h -> help listing serve,info,eval,run,logs,screenshot,cdp + nil error
  run|logs|screenshot|cdp without session -> error names both session sources
```
