...
// best-effort launch: new window + session URL only.
//
...
// openFirefox launches default-profile Firefox with a new window to sessionURL.
...
func launchFirefoxWithArgs(args []string) error {
	// Prefer real binary so -new-window joins the existing Firefox session.
	// macOS: if binary missing, open -na --args as last resort.
	return launchBrowserArgs(ResolveFirefoxBinary, "Firefox", "Firefox", "firefox", args)
}
